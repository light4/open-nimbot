...
App 连接后会自动读取 RFID，并显示标签条码、序列号、介质类型与已用/总长度。介质类型 `1` 表示有间隙标签。条码 `6971501227682` 已映射为 30 × 15 mm / 2R 标签，即 203 dpi 下单张 240 × 120 px。2R 的上下两张是独立标签；编辑器会按物理的上、下顺序同时展示两块画布，分别编辑、预览，并作为两个页面一次打印。
