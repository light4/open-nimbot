...
画布将文本、图片和自由路径保存为独立图层。文本图层可从 macOS 已安装字体中选择字体、字号、粗体和斜体；图片通过 **Image…** 插入。选择图层后显示虚线边框，可拖拽移动、双指缩放、置前、置后或删除；开启 **Draw** 可绘制路径。**Copy Top → Bottom** 会复制全部图层。画布、预览和打印共用同一个 Core Graphics 栅格渲染器。
