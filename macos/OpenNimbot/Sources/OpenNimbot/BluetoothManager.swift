  ...
  CBPeripheralDelegate
{
  ...
  func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
    guard let service = peripheral.services?.first(where: { $0.uuid == NimbotProtocol.service })
    else { return }
  ...
  func peripheral(
    ...
    characteristic = service.characteristics?.first(where: {
      $0.uuid == NimbotProtocol.characteristic
    })
  ...
  func peripheral(
    ...
    guard error == nil, characteristic.uuid == NimbotProtocol.characteristic,
  ...
  func peripheral(
    ...
    while responseBuffer.count >= 7 {
      guard responseBuffer[responseBuffer.startIndex] == 0x55,
        responseBuffer[responseBuffer.startIndex + 1] == 0x55
      else {
  ...
  private func writeNext(to characteristic: CBCharacteristic) {
    ...
    connectedPrinter?.peripheral.writeValue(
      queue.removeFirst(), for: characteristic, type: .withResponse)
  }
}
