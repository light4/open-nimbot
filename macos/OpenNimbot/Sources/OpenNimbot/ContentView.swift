...
struct ContentView: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack {
        Text("OpenNimbot").font(.title2.bold())
        Spacer()
        printerMenu
      }
      HStack {
      ...
      Button("Print label pair") { bluetooth.print([top, bottom]) }.disabled(
        bluetooth.connectedPrinter == nil)
    }
  ...
  private func labelCanvas(_ title: String, document: CanvasDocument, index: Int) -> some View {
    VStack(alignment: .leading, spacing: 4) {
      ...
      CanvasEditor(
        document: document,
        size: CGSize(width: bluetooth.mediaProfile.width, height: bluetooth.mediaProfile.height),
        drawing: $drawing, onActivate: { selectedLabel = index }
      )
      .frame(height: 210)
    }
  }
  ...
  @ViewBuilder private var inspector: some View {
    if let item = canvas.selected, case .text(let value) = item.content {
      VStack(alignment: .leading, spacing: 4) {
        ...
        HStack {
          ...
          Stepper(
            "\(Int(item.font.pointSize)) pt", value: fontSizeBinding(item.id, item.font.pointSize),
            in: 8...48)
  ...
  private func preview(_ name: String, _ canvas: CanvasDocument) -> some View {
    VStack {
      ...
      Image(nsImage: NimbotProtocol.preview(canvas: canvas, media: bluetooth.mediaProfile))
        .resizable().interpolation(.none).scaledToFit().frame(width: 210)
    }
  }

  private func textBinding(_ id: UUID, _ value: String) -> Binding<String> {
    Binding(
      get: { value },
      set: { text in
        canvas.update(id) { $0.content = .text(text) }
        canvas.fitText(id)
      })
  }
  private func fontBinding(_ id: UUID, _ value: String) -> Binding<String> {
    Binding(
      get: { value },
      set: { family in
        canvas.update(id) {
          $0.font =
            family == "System"
            ? .systemFont(ofSize: $0.font.pointSize)
            : NSFontManager.shared.font(
              withFamily: family, traits: [], weight: 5, size: $0.font.pointSize) ?? $0.font
        }
        canvas.fitText(id)
      })
  }
  private func fontSizeBinding(_ id: UUID, _ value: CGFloat) -> Binding<Double> {
    Binding(
      get: { Double(value) },
      set: { size in
        canvas.update(id) { $0.font = NSFontManager.shared.convert($0.font, toSize: CGFloat(size)) }
        canvas.fitText(id)
      })
  }
  private func boldBinding(_ id: UUID, _ value: Bool) -> Binding<Bool> {
    Binding(
      get: { value },
      set: { enabled in
        canvas.update(id) { $0.bold = enabled }
        canvas.fitText(id)
      })
  }
  private func italicBinding(_ id: UUID, _ value: Bool) -> Binding<Bool> {
    Binding(
      get: { value },
      set: { enabled in
        canvas.update(id) { $0.italic = enabled }
        canvas.fitText(id)
      })
  }
  private func alignmentBinding(_ id: UUID, _ value: NSTextAlignment) -> Binding<NSTextAlignment> {
    Binding(get: { value }, set: { alignment in canvas.update(id) { $0.alignment = alignment } })
  }

  private var printerMenu: some View {
    Menu {
      Button("Scan", action: bluetooth.scan)
      ForEach(bluetooth.printers) { printer in Button(printer.name) { bluetooth.connect(printer) } }
      if bluetooth.connectedPrinter != nil { Button("Refresh media", action: bluetooth.readMedia) }
    } label: {
      Label(bluetooth.connectedPrinter?.name ?? "Printer", systemImage: "printer")
    }
  }
}
